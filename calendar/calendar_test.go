...
func assertCalendarCacheRead(t *testing.T, service *Calendar, title string) {
	...
	if err != nil || value.Title != title {
		t.Fatalf(
			"calendar GetCalendar returned stale data: value=%+v err=%v",
			value,
			err,
		)
	}
...
func TestCalendarSequentialLifecycleAndCallback(t *testing.T) {
	...
	progress, err := service.User.GetProgress(
		ctx,
		user.GetProgressParams{Identity: identity, CalendarID: calendarID},
	)
	if err != nil || progress == nil || progress.ClaimCount != 2 ||
	...
	if err != nil || stats.OperationCount != 3 || stats.GrantCount != 2 ||
		stats.UniqueUsers != 1 {
...
func TestCalendarImportExportCycle(t *testing.T) {
	...
	if len(imported.Calendars) != 1 ||
		...
		len(imported.Calendars[0].Steps[0].Rewards) != 1 {
	...
	if len(replaced.Calendars) != 1 ||
		...
		t.Fatalf(
			"update_existing kept removed calendar children: %+v",
			replaced.Calendars,
		)
	}
}
...
func TestCalendarIntervalAndResetModes(t *testing.T) {
	...
	if !first.Granted || blocked.Status != repository.StatusNotAvailable ||
		...
		t.Fatalf(
			"interval results: first=%+v blocked=%+v second=%+v",
			first,
			blocked,
			second,
		)
...
func TestCalendarSequentialResetNextMatchesRecordAfterMissedDay(t *testing.T) {
	...
	if !nextDayPreview.Granted ||
		...
		*nextDayPreview.Position != 2 ||
		...
		t.Fatalf(
			"next-day preview must continue with day 2: %+v",
			nextDayPreview,
		)
	}
	...
	if !resetPreview.Granted ||
		...
		*resetPreview.Position != 1 ||
		...
		t.Fatalf(
			"preview after missed day must reset to day 1: %+v",
			resetPreview,
		)
	}
	...
	if !resetRecord.Granted ||
		...
		*resetRecord.Position != 1 ||
		...
		t.Fatalf(
			"record after missed day must match reset preview: %+v",
			resetRecord,
		)
	}
}
...
func TestCalendarConcurrentSingleIntervalGrant(t *testing.T) {
	...
	createStepReward(
		t,
		service,
		testsupport.WorkspaceID("workspace-concurrent"),
		calendarID,
		1,
		"coin",
		1,
...
func TestCalendarStatusesVisibilityAndAdminCRUD(t *testing.T) {
	...
	if value := record(
		...
		"inactive-op",
		now,
	); value.Status != repository.StatusInactive {
	...
	if value := record(
		...
		"future-op",
		now,
	); value.Status != repository.StatusNotStarted {
	...
	if value := record(
		...
		"expired-op",
		now,
	); value.Status != repository.StatusExpired {
	...
	if err != nil || !next.Granted || next.Position == nil ||
		*next.Position != 2 {
...
func TestCalendarAdminSurfaceAndCallbackControls(t *testing.T) {
	...
	calendarValue, err := service.Admin.GetCalendar(
		ctx,
		workspaceID,
		calendarID,
	)
...
func newCalendarTestServiceWithOptions(
	t testing.TB,
	options Options,
) *Calendar {
	t.Helper()
...
func terminateCalendarConnections(
	ctx context.Context,
	t testing.TB,
	db *sql.DB,
	database string,
