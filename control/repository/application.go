...
func (r *Repository) UpsertApplicationPlatform(
	...
	err = r.withAuditDBTx(ctx, func(tx *sql.Tx, q *controlsqlc.Queries) error {
		...
		row, err := q.UpsertApplicationPlatform(ctx, controlsqlc.UpsertApplicationPlatformParams{
...
func (r *Repository) ListApplicationPlatforms(
	...
	if err := r.requireWorkspaceAccess(ctx, normalizeID(actorID), workspaceID, "control.workspace.update"); err != nil {
		return nil, err
	}
...
func (r *Repository) DeleteApplicationPlatform(
	...
	workspaceID = normalizeID(workspaceID)
	if err := validateApplicationKey(workspaceID, appID, platformID); err != nil {
		return 0, err
	}

	var affected int64
	err := r.withAuditDBTx(ctx, func(tx *sql.Tx, q *controlsqlc.Queries) error {
		if err := requireWorkspaceAuthorization(ctx, q, normalizeID(actorID), workspaceID, "", "control.workspace.update"); err != nil {
			return err
		}
		var err error
		affected, err = q.DeleteApplicationPlatform(ctx, controlsqlc.DeleteApplicationPlatformParams{
...
func (r *Repository) GetApplicationAuthentication(
	...
	return sqlwrap.Query(ctx, r.db, sqlwrap.Params{
		KeyParts:          []any{"control", "application-auth", workspaceID, appID, platformID},
		Timeout:           r.timeout,
		CacheVersionScope: applicationCacheScope(workspaceID, appID, platformID),
		CacheL1Delay:      applicationCacheTTL,
		CacheL2Delay:      applicationCacheTTL,
	}, func(ctx context.Context) (ApplicationAuthentication, error) {
		row, err := r.q.GetApplicationPlatform(ctx, controlsqlc.GetApplicationPlatformParams{
			WorkspaceID: workspaceID,
			AppID:       appID,
			PlatformID:  platformID,
		})
...
func (r *Repository) requireWorkspaceAccess(ctx context.Context, actorID, workspaceID, methodKey string) error {

	return sqlwrap.WithTx(ctx, r.db.DB(), func(tx *sql.Tx) *controlsqlc.Queries {
		return controlsqlc.New(tx)
	}, func(_ *sql.Tx, q *controlsqlc.Queries) error {
		return requireWorkspaceAuthorization(ctx, q, actorID, workspaceID, "", methodKey)
	})
...
func validateApplicationPlatformInput(value ApplicationPlatformInput) error {
	...
	if err := validateApplicationKey(value.WorkspaceID, value.AppID, value.PlatformID); err != nil {
		return err
	}
	if value.Secret == "" || (value.Provider != ApplicationProviderVKMA && value.Provider != ApplicationProviderTMA) ||
		value.MaxAuthenticationAgeSeconds < 1 || value.MaxAuthenticationAgeSeconds > 86400 {
...
func mapApplicationPlatform(value controlsqlc.ControlApplicationPlatform) ApplicationPlatform {
...
func mapApplicationPlatformUpsert(value controlsqlc.UpsertApplicationPlatformRow) ApplicationPlatform {
...
func mapApplicationPlatformList(value controlsqlc.ListApplicationPlatformsRow) ApplicationPlatform {
...
func mapApplicationPlatformGet(value controlsqlc.ControlApplicationPlatform) ApplicationPlatform {
