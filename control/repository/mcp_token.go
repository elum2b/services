...
func (r *Repository) CreateMCPToken(
	...
	if len(value.Name) > 128 || (value.ExpiresAt != nil && !value.ExpiresAt.After(time.Now())) {
...
func (r *Repository) ListMCPTokens(ctx context.Context, accountID string) ([]MCPToken, error) {
...
func (r *Repository) RevokeMCPToken(ctx context.Context, accountID, tokenID string) (int64, error) {
	...
	var affected int64
	err := r.withAuditDBTx(ctx, func(tx *sql.Tx, q *controlsqlc.Queries) error {
		if err := lockAccountAuthentication(ctx, tx, normalizeID(accountID)); err != nil {
			return err
		}
		var err error
...
func (r *Repository) ValidateMCPToken(ctx context.Context, rawToken string) (MCPToken, error) {
