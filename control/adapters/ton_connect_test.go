...
func TestTONConnectReturnsAuthIdentityParams(t *testing.T) {
	...
	proof := TONConnectProof{
		...
		Domain: TONConnectDomain{
			LengthBytes: uint32(len("example.com")),
			Value:       "example.com",
		},
		Payload: "nonce-1",
	}

	proof.Signature = signTONConnectProof(
		t,
		privateKey,
		addr.Workchain(),
		addr.Data(),
	...
	if params.DisplayName != addr.StringRaw() || params.IP != "127.0.0.1" ||
		params.UserAgent != "ua" ||
		!params.BindToIP {
...
func signTONConnectProof(
	...
	return base64.StdEncoding.EncodeToString(
		ed25519.Sign(privateKey, fullHash[:]),
	)
}
