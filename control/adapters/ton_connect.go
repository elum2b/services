...
func TONConnect(
	...
	if err := validateTONDomain(
		params.Proof.Domain,
		expectedDomain,
		params.AllowNativeDomain,
	); err != nil {
		return admin.AuthIdentityParams{}, err
	}
	...
	if err != nil {
		return admin.AuthIdentityParams{}, serviceerrors.Wrap(
			serviceerrors.CodeUnauthorized,
			"ton connect signature is invalid",
			err,
		)
	}
	...
	if err != nil {
		return admin.AuthIdentityParams{}, serviceerrors.Wrap(
			serviceerrors.CodeUnauthorized,
			"ton connect state init is invalid",
			err,
		)
	}
	...
	verifier := wallet.NewTonConnectVerifier(
		expectedDomain,
		maxAge,
		params.Client,
	)
	if strings.TrimSpace(params.PayloadSecret) != "" {
		err = verifier.VerifyProofHandlePayload(
			ctx,
			addr,
			proof,
			stateInit,
			wallet.CheckPayload,
			params.PayloadSecret,
		)
	} else {
