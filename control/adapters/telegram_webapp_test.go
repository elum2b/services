...
func TestTelegramWebAppResolve(t *testing.T) {
	...
	rawData := signedTelegramWebAppData(t, "bot-token", url.Values{
		...
		"user": []string{
			`{"id":1093776793,"first_name":"Root","last_name":"Admin","username":"root"}`,
		},
	})

	identity, err := provider.Resolve(
		context.Background(),
	...
	if identity.Provider != ProviderTelegramWebApp ||
		identity.Subject != "1093776793" ||
		identity.DisplayName != "Root Admin" {
...
func TestTelegramWebAppFunctionReturnsAuthIdentityParams(t *testing.T) {
	...
	params, err := TelegramWebApp(
		context.Background(),
		TelegramWebAppAuthParams{
			BotToken:    "bot-token",
			InitData:    rawData,
	...
	if params.Provider != ProviderTelegramWebApp ||
		params.Subject != "1093776793" ||
		params.DisplayName != "Root" {
...
func TestTelegramWebAppRejectsInvalidSignature(t *testing.T) {
	...
	provider, err := NewTelegramWebApp(
		TelegramWebAppConfig{BotToken: "bot-token"},
	)
	if err != nil {
		t.Fatalf("new telegram webapp provider: %v", err)
	}
	...
	_, err = provider.Resolve(
		context.Background(),
		Request{
			RawData: `auth_date=1700000000&user=%7B%22id%22%3A1%7D&hash=bad`,
		},
	)
	if err == nil {
