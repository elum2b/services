...
func TestNormalizeTableName(t *testing.T) {
	...
	if got := normalizeTableName("abcdefghijklmnopqrstuvwxyzabcdefghijklmno"); got != DefaultTable {
...
func TestStoreQueryTemplatesAreInitializedOnce(t *testing.T) {
	...
	if !strings.Contains(first.queue, `INSERT INTO "jobs"`) || !strings.Contains(first.queue, `INSERT INTO "jobs_history"`) {
		t.Fatalf("queue template does not contain quoted tables: %q", first.queue)
	}
	s.table = "other_jobs"
...
func TestValidateIdentity(t *testing.T) {
	if err := validateIdentity("service", "c2b604c6-6960-41a7-b330-5083ca633434"); err != nil {
		t.Fatal(err)
	}
	if err := validateIdentity("", "c2b604c6-6960-41a7-b330-5083ca633434"); err == nil {
...
func TestJobStatesAreDistinct(t *testing.T) {
	states := map[string]bool{StatusQueued: true, StatusProcessing: true, StatusCompleted: true, StatusFailed: true}
	if len(states) != 4 {
...
func TestHandleExportRejectsNilDump(t *testing.T) {
	manager := &Manager{handler: nilDumpHandler{}}
	if err := manager.handleExport(t.Context(), Job{}); err == nil {
		t.Fatal("nil export dump accepted")
	}
}

func TestNewTokenIsUnique(t *testing.T) {
	if first, second := newToken(), newToken(); first == second || first == "" || second == "" {
...
func TestDiskArchiveStoresOpaqueArchive(t *testing.T) {
	...
	key, err := archive.Store(t.Context(), ArchiveObject{FileName: "../unsafe.zip"}, strings.NewReader("archive"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := os.Stat(filepath.Join(archive.directory, key)); !os.IsNotExist(err) {
...
func TestStartIsIdempotent(t *testing.T) {
	...
	cancel()
	manager := &Manager{workers: goroutine.New(), rootCtx: t.Context(), cancel: func() {}, idleDelay: time.Millisecond, cleanupPeriod: time.Hour}
...
type nilDumpHandler struct{}

func (nilDumpHandler) Export(context.Context, Job) (io.ReadCloser, error) {
	return nil, nil
}

func (nilDumpHandler) Import(context.Context, Job, io.Reader) error { return nil }
