...
func (r *Repository) UpsertGroup(
	...
	if err := r.withWorkspaceMutation(
		...
		func(txRepo *Repository) error {
			return txRepo.q.AdminUpsertGroup(
				ctx,
				tasksqlc.AdminUpsertGroupParams{
...
func (r *Repository) UpsertGroupLocalization(
	...
	if strings.TrimSpace(key) == "" ||
		...
		return fmt.Errorf(
			"tasks group localization scope, locale, and title are required",
		)
	}
	...
	if err := r.withWorkspaceMutation(
		...
		workspaceID,
		func(txRepo *Repository) error {
			return txRepo.q.AdminUpsertGroupLocalization(
				ctx,
				tasksqlc.AdminUpsertGroupLocalizationParams{
					WorkspaceID: workspaceID,
					GroupKey:    key,
					Locale:      locale,
					Title:       title,
...
func (r *Repository) UpsertSequence(
	ctx context.Context,
	workspaceID, key string,
	position int32,
	active bool,
	...
	if err := r.withWorkspaceMutation(
		...
		workspaceID,
		func(txRepo *Repository) error {
			return txRepo.q.AdminUpsertSequence(
				ctx,
				tasksqlc.AdminUpsertSequenceParams{
					WorkspaceID: workspaceID,
					Key:         key,
					Position:    position,
					IsActive:    active,
...
func (r *Repository) ListSequences(
	ctx context.Context,
	workspaceID string,
) ([]Sequence, error) {
	if err := requireWorkspaceID(workspaceID); err != nil {
...
func (r *Repository) DeleteSequence(
	ctx context.Context,
	workspaceID, key string,
) (int64, error) {
	var rows int64
...
func (r *Repository) SaveTask(
			...
			func(txRepo *Repository) error {
				...
				id, err = txRepo.q.AdminCreateTask(
					...
					tasksqlc.AdminCreateTaskParams{
						...
						TargetCount:     int64(params.TargetCount),
						ResetUnit:       params.ResetUnit,
						ResetEvery:      int32(params.ResetEvery),
						Position:        params.Position,
						Payload:         rawMessageParam(params.Payload),
						Target:          rawMessageParam(params.Target),
						IntegrationKind: nullString(params.IntegrationKind),
						IntegrationProvider: nullString(
							params.IntegrationProvider,
						),
						IntegrationPayload: rawMessageParam(
							params.IntegrationPayload,
						),
	...
	err := r.withWorkspaceMutation(
		...
		params.WorkspaceID,
		func(txRepo *Repository) error {
			_, err := txRepo.q.AdminUpdateTask(
				ctx,
				tasksqlc.AdminUpdateTaskParams{
					GroupKey:    params.GroupKey,
					SequenceKey: nullString(params.SequenceKey),
					SequencePosition: nullInt32FromUint32(
						params.SequencePosition,
					),
					TaskKind:        params.TaskKind,
					...
					IsVisible:   params.IsVisible,
					IsActive:    params.IsActive,
					StartAt:     nullTime(params.StartAt),
					EndAt:       nullTime(params.EndAt),
					WorkspaceID: params.WorkspaceID,
					ID:          int64(params.ID),
				},
	...
	out, err := repositoryQuery[[]Task](ctx, r, sqlwrap.Params{
		...
		if groupKey != "" {
			rows, err := r.q.AdminListTasksByGroup(
				ctx,
				tasksqlc.AdminListTasksByGroupParams{
					WorkspaceID: workspaceID,
					GroupKey:    groupKey,
					Limit:       limit,
					Offset:      offset,
...
func (r *Repository) UpsertTaskLocalization(
	...
	if taskID == 0 || taskID > math.MaxInt64 ||
		strings.TrimSpace(locale) == "" || strings.TrimSpace(title) == "" {
		return fmt.Errorf(
			"tasks localization scope, locale, or title is invalid",
		)
	}
	...
	if err := r.withWorkspaceMutation(
		...
		workspaceID,
		func(txRepo *Repository) error {
			return txRepo.q.AdminUpsertTaskLocalization(
				ctx,
				tasksqlc.AdminUpsertTaskLocalizationParams{
					WorkspaceID: workspaceID,
					TaskID:      int64(taskID),
					Locale:      locale,
					Title:       title,
...
func (r *Repository) GetGroupLocalization(
	ctx context.Context,
	workspaceID, key, locale string,
) (Localization, error) {
	row, err := r.q.AdminGetGroupLocalization(
		ctx,
		tasksqlc.AdminGetGroupLocalizationParams{
			WorkspaceID: workspaceID,
			GroupKey:    key,
			Locale:      locale,
		},
...
func (r *Repository) ListGroupLocalizations(
	ctx context.Context,
	workspaceID, key string,
) ([]Localization, error) {
	rows, err := r.q.AdminListGroupLocalizationsByGroup(
		ctx,
		tasksqlc.AdminListGroupLocalizationsByGroupParams{
			WorkspaceID: workspaceID,
			GroupKey:    key,
		},
	)
...
func (r *Repository) DeleteGroupLocalization(
	ctx context.Context,
	workspaceID, key, locale string,
) (int64, error) {
	var rows int64
...
func (r *Repository) GetTaskLocalization(
	ctx context.Context,
	workspaceID string,
	taskID uint64,
	locale string,
) (Localization, error) {
	row, err := r.q.AdminGetTaskLocalization(
		ctx,
		tasksqlc.AdminGetTaskLocalizationParams{
			WorkspaceID: workspaceID,
			TaskID:      int64(taskID),
			Locale:      locale,
		},
...
func (r *Repository) ListTaskLocalizations(
	ctx context.Context,
	workspaceID string,
	taskID uint64,
) ([]Localization, error) {
	rows, err := r.q.AdminListTaskLocalizationsByTask(
		ctx,
		tasksqlc.AdminListTaskLocalizationsByTaskParams{
			WorkspaceID: workspaceID,
			TaskID:      int64(taskID),
		},
	)
...
func (r *Repository) DeleteTaskLocalization(
	ctx context.Context,
	workspaceID string,
	taskID uint64,
	locale string,
...
func (r *Repository) GetReward(
	...
	row, err := r.q.AdminGetReward(
		ctx,
		tasksqlc.AdminGetRewardParams{
			WorkspaceID: workspaceID,
			TaskID:      int64(taskID),
			RewardKey:   key,
		},
...
func (r *Repository) ListRewards(
	...
	rows, err := r.q.AdminListRewardsByTask(
		ctx,
		tasksqlc.AdminListRewardsByTaskParams{
			WorkspaceID: workspaceID,
			TaskID:      int64(taskID),
		},
	)
...
func (r *Repository) GetComplexCondition(
	ctx context.Context,
	workspaceID string,
	parentTaskID, conditionTaskID uint64,
) (ComplexCondition, error) {
	row, err := r.q.AdminGetComplexCondition(
		ctx,
		tasksqlc.AdminGetComplexConditionParams{
			WorkspaceID:     workspaceID,
	...
	if err := r.withWorkspaceMutation(
		...
		func(txRepo *Repository) error {
			return txRepo.q.AdminUpsertReward(
				ctx,
				tasksqlc.AdminUpsertRewardParams{
					WorkspaceID: workspaceID,
					TaskID:      int64(taskID),
...
func (r *Repository) UpsertComplexCondition(
	ctx context.Context,
	params SaveComplexConditionParams,
) error {
	params.RequiredStatus = defaultString(
	...
	err := r.WithTx(ctx, func(txRepo *Repository) error {
		...
		condition, err := txRepo.q.AdminGetTask(
			ctx,
			tasksqlc.AdminGetTaskParams{
				WorkspaceID: params.WorkspaceID,
		...
		rows, err := txRepo.q.AdminListComplexConditions(
			ctx,
			params.WorkspaceID,
		)
		if err != nil {
		...
		return txRepo.q.AdminUpsertComplexCondition(
			ctx,
			tasksqlc.AdminUpsertComplexConditionParams{
				WorkspaceID:     params.WorkspaceID,
...
func (r *Repository) ListComplexConditions(
	ctx context.Context,
	workspaceID string,
) ([]ComplexCondition, error) {
	if err := requireWorkspaceID(workspaceID); err != nil {
