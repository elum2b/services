...
func (r *Repository) GetStats(
	...
	return Stats{
		...
		ActiveTasks: uint64(row.ActiveTasks),
		VisibleTasks: uint64(
			row.VisibleTasks,
		),
		...
		ReadyProgress: uint64(row.ReadyProgress),
		ClaimedProgress: uint64(
			row.ClaimedProgress,
		),
		ProgressCreated: uint64(row.ProgressCreated),
		ProgressAmount: uint64(
			row.ProgressAmount,
		),
		ReadyCount: uint64(row.ReadyCount),
		ClaimedCount: uint64(
			row.ClaimedCount,
		),
		ManualClaimedCount: uint64(row.ManualClaimedCount),
		AutoClaimedCount: uint64(
			row.AutoClaimedCount,
		),
		UniqueParticipants: uint64(row.UniqueParticipants),
...
func (r *Repository) GetSingleTaskStats(
	...
	row, err := r.q.AdminGetSingleTaskStats(
		ctx,
		tasksqlc.AdminGetSingleTaskStatsParams{
...
func (r *Repository) ListDailyStats(
	...
	rows, err := r.q.AdminListTaskDailyStats(
		ctx,
		tasksqlc.AdminListTaskDailyStatsParams{
			WorkspaceID: workspaceID, TaskID: int64(taskID),
	...
	for _, row := range rows {
		result = append(result, DailyStats{
			...
			ProgressAmount: uint64(row.ProgressAmount),
			ReadyCount: uint64(
				row.ReadyCount,
			),
			ClaimedCount: uint64(row.ClaimedCount),
			ManualClaimedCount: uint64(
				row.ManualClaimedCount,
			),
			AutoClaimedCount: uint64(row.AutoClaimedCount),
			UniqueParticipants: uint64(
...
func (r *Repository) ListDailyOverview(
	...
	rows, err := r.q.AdminListTaskDailyOverview(
		ctx,
		tasksqlc.AdminListTaskDailyOverviewParams{
			WorkspaceID:   workspaceID,
			StatsDate:     from,
			StatsDate_2:   until,
			WorkspaceID_2: workspaceID,
	...
	for _, row := range rows {
		result = append(result, DailyOverview{
			Date:       row.StatsDate,
			TasksTotal: uint64(row.TasksTotal),
			ActiveTasks: uint64(
				row.ActiveTasks,
			),
			VisibleTasks: uint64(row.VisibleTasks),
			ProgressCreated: uint64(
				row.ProgressCreated,
			),
			ProgressAmount: uint64(row.ProgressAmount),
...
func (r *Repository) RefreshDailyStats(
	...
	if err := r.q.RefreshTaskDailyStats(
		ctx,
		tasksqlc.RefreshTaskDailyStatsParams{
