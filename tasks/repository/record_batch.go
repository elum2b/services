...
func compileProgressBulkUpsert(
	identity Identity,
	items []recordProgressUpsert,
) (string, []any) {
	const columns = 11
	...
	builder.WriteString(
		"progress = LEAST(task_progress.progress + EXCLUDED.progress, ",
	)
	builder.WriteString(
	...
	builder.WriteString(
		"WHEN task_progress.status IN ('ready', 'claimed') THEN task_progress.status ",
	)
	...
	builder.WriteString(
		"WHEN task_progress.ready_at IS NOT NULL THEN task_progress.ready_at ",
	)
	builder.WriteString("WHEN task_progress.progress + EXCLUDED.progress >= ")
	builder.WriteString(
