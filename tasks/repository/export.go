...
func (r *Repository) ExportManifest() ExportManifest {
	return ExportManifest{
		...
		Sections: []ExportManifestSection{
			{
				Key:            ExportSectionGroups,
				Title:          "Groups",
				Description:    "Task groups and ordering.",
				DefaultEnabled: true,
			},
			{
				Key:            ExportSectionTasks,
				Title:          "Tasks",
...
func (r *Repository) exportSnapshot(
	ctx context.Context,
	workspaceID string,
	req ExportRequest,
) (ExportPackage, error) {
	...
	if sections[ExportSectionRewards] {
		rewardRows, err = repositoryValue(
			ctx,
			r,
			func(ctx context.Context) ([]tasksqlc.TaskReward, error) {
	...
	for _, reward := range rewardRows {
		...
		rewardsByTaskID[taskID] = append(rewardsByTaskID[taskID], ExportReward{
			...
			Unit:     taskDurationUnitPtr(reward.DurationUnit),
	...
	for _, row := range taskRows {
		...
		out.Groups[index].Tasks = append(out.Groups[index].Tasks, ExportTask{
			...
			Reset: ExportReset{
				Unit:  task.ResetUnit,
				Every: task.ResetEvery,
			},
			Position:     task.Position,
			Payload:      nullableRaw(task.Payload),
			Target:       target,
			...
			EndAt:        task.EndAt,
			Localization: taskLocalizationsByID[task.ID],
