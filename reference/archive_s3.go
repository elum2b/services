...
func newS3ArchiveWithClient(client archiveObjectClient, bucket string) (*s3Archive, error) {
...
func (a *s3Archive) Store(ctx context.Context, _ jobs.ArchiveObject, source io.Reader) (string, error) {
	if a == nil || source == nil {
...
func (a *s3Archive) Open(ctx context.Context, key string) (io.ReadCloser, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	...
	output, err := a.client.GetObject(ctx, &awss3.GetObjectInput{Bucket: aws.String(a.bucket), Key: aws.String(key)})
	if err != nil {
...
func (a *s3Archive) Delete(ctx context.Context, key string) error {
	...
	if _, err := a.client.DeleteObject(ctx, &awss3.DeleteObjectInput{Bucket: aws.String(a.bucket), Key: aws.String(key)}); err != nil {
