...
var (
	ErrInvalidPrice = serviceerrors.New(
		serviceerrors.CodeInvalidFields,
		"payment price is invalid",
	)
	...
	ErrInvalidProduct = serviceerrors.New(
		serviceerrors.CodeInvalidFields,
		"payment product is invalid",
	)
)
...
func (r *PaymentRepository) UpsertProductGroup(
	ctx context.Context,
	params ProductGroupUpsertParams,
) error {
	...
	err = r.withWorkspaceMutation(
		...
		func(tx *PaymentRepository) error {
			return tx.q.UpsertProductGroup(
				ctx,
				paymentsqlc.UpsertProductGroupParams{
					WorkspaceID: workspaceID,
					Code:        params.Code,
					TitleKey: sqlwrap.NullFromPtr(
						params.TitleKey,
						func(v string) sql.NullString {
							return sql.NullString{String: v, Valid: true}
						},
					),
					DescriptionKey: sqlwrap.NullFromPtr(
...
func (r *PaymentRepository) DeleteProductGroup(
	ctx context.Context,
	workspaceID string,
	code string,
) (int64, error) {
...
func (r *PaymentRepository) UpsertProduct(
	ctx context.Context,
	...
	if strings.TrimSpace(params.ID) == "" ||
		...
		params.GlobalLimit < 0 ||
		params.GlobalIntervalCount < 0 ||
		params.UserLimit < 0 ||
	...
	if globalInterval == "" {
		globalInterval = string(
			paymentsqlc.PaymentProductGlobalIntervalUNLIMITED,
		)
	}
	...
	return r.withWorkspaceMutation(
		...
		func(tx *PaymentRepository) error {
			if err := tx.q.UpsertProduct(ctx, paymentsqlc.UpsertProductParams{
				...
				ImageUrl: sqlwrap.NullFromPtr(
					params.ImageURL,
					func(v string) sql.NullString {
						return sql.NullString{String: v, Valid: true}
					},
				),
				LinkUrl: sqlwrap.NullFromPtr(
					params.LinkURL,
					func(v string) sql.NullString {
						return sql.NullString{String: v, Valid: true}
					},
				),
				SizeLabel: sqlwrap.NullFromPtr(
					params.SizeLabel,
					func(v string) sql.NullString {
						return sql.NullString{String: v, Valid: true}
					},
				),
				PeriodSeconds: sqlwrap.NullFromPtr(
					params.PeriodSeconds,
					func(v int64) sql.NullInt64 {
						return sql.NullInt64{Int64: v, Valid: true}
					},
				),
				TrialDurationSeconds: sqlwrap.NullFromPtr(
					params.TrialDurationSeconds,
					func(v int64) sql.NullInt64 {
						return sql.NullInt64{Int64: v, Valid: true}
					},
				),
				QuantityMode: paymentsqlc.PaymentProductQuantityMode(
					quantityMode,
				),
				Position:    params.Position,
				GlobalLimit: params.GlobalLimit,
				GlobalInterval: paymentsqlc.PaymentProductGlobalInterval(
					globalInterval,
				),
				GlobalIntervalCount: params.GlobalIntervalCount,
				UserLimit:           params.UserLimit,
				UserInterval: paymentsqlc.PaymentProductUserInterval(
					userInterval,
				),
...
func (r *PaymentRepository) DeleteProduct(
	ctx context.Context,
	workspaceID string,
	id string,
) (int64, error) {
...
func (r *PaymentRepository) UpsertLocalization(
	ctx context.Context,
	params LocalizationUpsertParams,
) error {
	workspaceID, err := requireWorkspaceID(params.WorkspaceID)
	if err != nil {
		return err
	...
	return r.withWorkspaceMutation(
		...
		workspaceID,
		func(tx *PaymentRepository) error {
			if err := tx.q.UpsertLocalization(
				ctx,
...
func (r *PaymentRepository) UpsertProductItem(
	ctx context.Context,
	params ProductItemUpsertParams,
) error {
	workspaceID, err := requireWorkspaceID(params.WorkspaceID)
	if err != nil {
		return err
	...
	return r.withWorkspaceMutation(
		...
		workspaceID,
		func(tx *PaymentRepository) error {
			if err := tx.q.UpsertProductItem(
				ctx,
				paymentsqlc.UpsertProductItemParams{
					WorkspaceID: workspaceID,
					ProductID:   params.ProductID,
					ItemID:      params.ItemID,
					RewardType: paymentsqlc.PaymentProductItemRewardType(
						rewardType,
					),
					Quantity: params.Quantity,
					Scale:    int16(params.Scale),
					DurationUnit: paymentsqlc.NullPaymentProductItemDurationUnit{
						PaymentProductItemDurationUnit: paymentsqlc.PaymentProductItemDurationUnit(
							pointerString(params.DurationUnit),
						),
...
func (r *PaymentRepository) CreateProductPrice(
	ctx context.Context,
	...
	err = r.withWorkspaceMutation(
		...
		func(tx *PaymentRepository) error {
			...
			if amounts.dynamic {
				...
				id, err = tx.q.CreateProductPrice(
					ctx,
...
func (r *PaymentRepository) UpdateProductPrice(
	ctx context.Context,
	...
	err = r.withWorkspaceMutation(
		...
		workspaceID,
		func(tx *PaymentRepository) error {
			productID, err := tx.q.GetProductPriceProductID(
				ctx,
				paymentsqlc.GetProductPriceProductIDParams{
					WorkspaceID: workspaceID,
					ID:          int64(params.ID),
				},
			)
			...
			if amounts.dynamic {
				...
				rows, err = tx.q.UpdateProductPrice(
					ctx,
...
func (r *PaymentRepository) DeleteProductPrice(
	ctx context.Context,
	workspaceID string,
	id uint64,
) (int64, error) {
