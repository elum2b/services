...
func (a *TON) CreateTransaction(
	...
	if responseDestination == "" {
		...
		responseDestination, err = normalizeTONAddress(
			responseDestination,
			network,
			ErrResponseAddressInvalid,
			ErrResponseAddressInvalid,
		)
		if err != nil {
			return nil, err
...
func transactionJetton(
	...
	amountMinor uint64,
	comment string,
	asset paymentsqlc.PaymentAsset,
) (*Transaction, error) {
	if !asset.ContractAddress.Valid ||
		strings.TrimSpace(asset.ContractAddress.String) == "" {
	...
	if err != nil {
		return nil, serviceerrors.Wrap(
			...
			ErrDestinationAddressInvalid.Message(),
			err,
		)
	}
	...
	if err != nil {
		return nil, serviceerrors.Wrap(
			serviceerrors.CodeInvalidFields,
			ErrResponseAddressInvalid.Message(),
			err,
		)
	}

	jettonWalletAddress, err := resolveJettonWalletAddress(
		ctx,
		network,
		networkConfigURL,
		sourceWallet,
...
func resolveJettonWalletAddress(
	ctx context.Context,
	network string,
	networkConfigURL string,
	ownerWallet string,
	...
	if err != nil {
		return "", serviceerrors.Wrap(
			...
			ErrNetworkConfigLoadFailed.Message(),
			err,
		)
	}
	...
	if err := client.AddConnectionsFromConfig(stickyCtx, cfg); err != nil {
		return "", serviceerrors.Wrap(
			serviceerrors.CodeUnavailable,
			ErrLiteConnectionsFailed.Message(),
			err,
		)
	}
	...
	if err != nil {
		return "", serviceerrors.Wrap(
			serviceerrors.CodeInvalidFields,
			ErrMasterAddressInvalid.Message(),
			err,
		)
	}
	...
	if err != nil {
		return "", serviceerrors.Wrap(
			serviceerrors.CodeUnavailable,
			ErrRecipientWalletResolveFailed.Message(),
			err,
		)
	}
