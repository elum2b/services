...
При импорте dynamic-цены поля `list_amount_minor` и `discount_amount_minor` считаются производными и пересчитываются из reference-полей по актуальному глобальному курсу. Перед записью `PreflightImport` сообщает обязательные assets, dynamic rates и, при подключенном через `payment.Options.ReferenceItemChecker` `ReferenceItemDependencyChecker`, keys Reference item-ов. `Import` повторяет preflight под workspace lock и не меняет каталог при отсутствующих или `pending` dependencies. Ошибка `ImportDependenciesMissingError` содержит полный report. Импорт не доверяет сохраненному target amount из другого сервера. Pending-курсы также не выдаются пользователю и не используются при создании dynamic-цены.
...
- инициатор и получатель восстанавливаются как `Identity{WorkspaceID, AppID, PlatformID, PlatformUserID}`;
...
`Admin.GetPaymentReport` возвращает страницу платежей и статистику по тому же
набору фильтров. Пустой фильтр не ограничивает выборку; пустая сортировка
использует стабильный порядок `created_at DESC, id DESC`.

Это единая точка для текущей агрегированной статистики: количества заказов по
всем статусам, завершенных покупок, купленного количества, уникальных
покупателей, gross/refund/net и разбивки по assets. Для статистики одного
продукта используется тот же метод с `ProductID`; отдельных `GetStats` и
`GetProductStats` нет. Дневные временные ряды остаются отдельными методами,
поскольку возвращают группировку по датам, а не единый агрегат.

Каждая строка содержит две полные identity:

- `Initiator` — пользователь, который начал и оплачивает покупку;
- `Recipient` — пользователь, которому принадлежит товар или награда.

Для обычной покупки identity совпадают, для подарка различаются. Пользователь
всегда scoped по `WorkspaceID + AppID + PlatformID + PlatformUserID`.
