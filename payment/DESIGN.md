...
`payment_order` - бизнес-заказ на покупку конкретного продукта конкретным
пользователем. Получатель хранится через
`workspace_id + app_id + platform_id + platform_user_id`; отдельные payer-поля
фиксируют инициатора подарочной покупки. Для обычной покупки инициатор совпадает
с получателем. В заказ копируется снимок цены: `list_amount_minor`,
`discount_amount_minor`, `payable_amount_minor`, `price_id`, `asset_code`,
`locale`.
...
Публичный метод `Product.GetByKey` возвращает только информацию о товаре и
лимитах. Лимиты считаются по получателю ключа, а не по покупателю. Созданный
order сохраняет payer отдельно от recipient; admin report возвращает их как
`Initiator` и `Recipient`.
