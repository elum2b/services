...
var (
	...
	ErrCallbackHandlerNil = serviceerrors.New(
		serviceerrors.CodeInvalidFields,
		"payment callback handler is nil",
	)
	...
	ErrCallbacksRegistrationClosed = serviceerrors.New(
		...
		"payment callbacks must be registered before Run",
	)
	...
	ErrCreateClient = serviceerrors.New(
	...
	ErrBootstrapFailed = serviceerrors.New(
		serviceerrors.CodeInternalError,
		"payment bootstrap failed",
	)
	// ErrDexScreenerClientRequired means the rate loader was called without an HTTP client.
	ErrDexScreenerClientRequired = serviceerrors.New(
		serviceerrors.CodeInvalidFields,
		"payment dexscreener HTTP client is nil",
	)
	// ErrDexScreenerAddressesRequired means there were no token addresses to request.
	ErrDexScreenerAddressesRequired = serviceerrors.New(
		serviceerrors.CodeInvalidFields,
		"payment dexscreener source token addresses are required",
	)
	// ErrDexScreenerBatchTooLarge means a single DexScreener batch exceeded the supported size.
	ErrDexScreenerBatchTooLarge = serviceerrors.New(
		serviceerrors.CodeInvalidFields,
		"payment dexscreener batch exceeds 30 token addresses",
	)
	// ErrUSDPriceInvalid means a fetched USD price cannot be parsed or is non-positive.
	ErrUSDPriceInvalid = serviceerrors.New(
		serviceerrors.CodeInvalidFields,
		"payment USD price is invalid",
	)
	// ErrUSDPriceOverflow means a fetched USD price exceeds the supported minor-unit range.
	ErrUSDPriceOverflow = serviceerrors.New(
		serviceerrors.CodeInvalidFields,
		"payment USD price exceeds supported range",
	)
	...
	ErrTelegramStarsRefundCredentialsRequired = serviceerrors.New(
		serviceerrors.CodeInvalidFields,
		"payment refund telegram_stars credentials are required",
	)
	ErrTelegramStarsFullRefundOnly = serviceerrors.New(
		serviceerrors.CodeFailedPrecondition,
		"payment refund telegram_stars supports full refunds only",
	)
	ErrTelegramStarsChargeIDRequired = serviceerrors.New(
		serviceerrors.CodeInvalidFields,
		"payment refund telegram_stars provider charge id is required",
	)
	ErrTelegramStarsPlatformUserIDInvalid = serviceerrors.New(
		serviceerrors.CodeInvalidFields,
		"payment refund telegram_stars platform user id must be int64",
	)
	ErrYooKassaRefundCredentialsRequired = serviceerrors.New(
		serviceerrors.CodeInvalidFields,
		"payment refund yookassa credentials are required",
	)
	ErrYooKassaPaymentIDRequired = serviceerrors.New(
		serviceerrors.CodeInvalidFields,
		"payment refund yookassa provider payment id is required",
	)
	ErrPlategaRefundParamsRequired = serviceerrors.New(
		serviceerrors.CodeInvalidFields,
		"payment refund platega parameters are required",
	)
	ErrTONRefundParamsRequired = serviceerrors.New(
		serviceerrors.CodeInvalidFields,
		"payment refund ton parameters are required",
	)
)
